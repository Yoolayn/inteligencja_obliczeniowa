# data rozpoczęcia: 2024-02-21 16:22
import math
...
if __name__ == "__main__":
    ...
    tomorrow = biorythm(
        lambda: date_of_birth - (datetime.now() + timedelta(1))
    )

    print("todays waves are:")
    for k in today:
        print()
        print(f"{k}: {today[k]}")
        if today[k] < -0.5:
            print(
                f"I am sorry you are having a bad {k} wave today {name}"
            )
            if today[k] < tomorrow[k]:
                print("tomorrow it will be better!")
        elif today[k] > 0.5:
            print(
                f"I must congratulate on having such a great {k} wave today {name}!"
            )
# data zakończenia: 17:37; 1h15min
